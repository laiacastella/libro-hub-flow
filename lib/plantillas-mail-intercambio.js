...
// Email 5: Notificación de reporte de problema en intercambio - enviar a admin/librohubflow
export async function enviarEmailReporteProblema(idIntercambio, idUsuarioReporta, tipoAccion, descripcion) {
    try {
        const [info] = await db.query(
            `SELECT 
                i.id_usuario_envia,
                ...
                i.id_libro_ofrecido,
                u_envia.nombre AS nombre_envia,
                u_envia.nick_usuario AS nick_envia,
                u_envia.email AS email_envia,
                u_recibe.nombre AS nombre_recibe,
                u_recibe.nick_usuario AS nick_recibe,
                u_recibe.email AS email_recibe,
                l_solicitado.titulo AS titulo_solicitado,
                l_ofrecido.titulo AS titulo_ofrecido
            FROM intercambios i
        ...
        const intercambio = info[0];
        const esUsuarioEnvia = Number(idUsuarioReporta) === Number(intercambio.id_usuario_envia);
        const nombreReporta = esUsuarioEnvia ? intercambio.nombre_envia : intercambio.nombre_recibe;
        const nickReporta = esUsuarioEnvia ? intercambio.nick_envia : intercambio.nick_recibe;
        const emailReporta = esUsuarioEnvia ? intercambio.email_envia : intercambio.email_recibe;
        const nombreOtro = esUsuarioEnvia ? intercambio.nombre_recibe : intercambio.nombre_envia;
        const emailOtro = esUsuarioEnvia ? intercambio.email_recibe : intercambio.email_envia;

        const contenido = `
            <p style="font-size: 16px; line-height: 1.6;">
                Se ha reportado un problema en un intercambio.
            </p>
            
            <div style="background-color: #ffe6e0; padding: 15px; border-radius: 10px; margin: 20px 0; border-left: 4px solid #ff3c00;">
                <p style="margin: 5px 0; font-size: 15px;">
                    <strong>Intercambio ID:</strong> #${idIntercambio}
                </p>
                <p style="margin: 5px 0; font-size: 15px;">
                    <strong>Usuario que reporta:</strong> ${nombreReporta} (${nickReporta || "Sin nick"}) — ${emailReporta}
                </p>
                <p style="margin: 5px 0; font-size: 15px;">
                    <strong>Otro usuario:</strong> ${nombreOtro} — ${emailOtro}
                </p>
                <p style="margin: 5px 0; font-size: 15px;">
                    <strong>Libro solicitado:</strong> "${intercambio.titulo_solicitado}"
                </p>
                <p style="margin: 5px 0; font-size: 15px;">
                    <strong>Libro ofrecido:</strong> "${intercambio.titulo_ofrecido || "No especificado"}"
                </p>
                <p style="margin: 5px 0; font-size: 15px;">
                    <strong>Tipo de acción:</strong> ${tipoAccion === "revertir" ? "Revertir y liberar libro" : "Cerrar y archivar libro"}
                </p>
                <p style="margin: 5px 0; font-size: 15px;">
                    <strong>Descripción del problema:</strong>
                </p>
                <p style="margin: 5px 0; font-size: 15px; font-style: italic; color: #555; background: #f9f9f9; padding: 10px; border-radius: 6px;">
                    "${descripcion}"
                </p>
            </div>
        ...
        await transporter.sendMail({
            ...
            subject: `⚠️ Problema reportado en intercambio #${idIntercambio}`,
            html: crearPlantillaEmail("Reporte de problema en intercambio", contenido),
        });
    } catch (e) {
        console.error("Error al enviar email de reporte de problema:", e.message);
    }
}

// Email 6: Notificación de nueva valoración recibida
